...
tsk_pam_tx()
{
	int16_t pulse_arr[LEN_INT16_FRAME];
	int16_t shaped_data[LEN_INT16_FRAME];
	int16_t dl[LEN_DL];

	memset(dl, 0, sizeof(int16_t) * LEN_DL);
	memset(pulse_arr, 0, sizeof(int16_t) * LEN_INT16_FRAME);
	...
	while(1)
	{
		...
		// Shape data
		fir_filter(pulse_arr, LEN_INT16_FRAME, rrc_h, LEN_H, shaped_data, dl);
